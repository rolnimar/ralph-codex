...
Default is 10 iterations. `./scripts/ralph/ralph.sh` uses Codex by default. Use `--tool amp`, `--tool codex`, or `--tool claude` to override the AI coding tool.
...
| File | Purpose |
|------|---------|
| `ralph.sh` | The bash loop that spawns fresh AI instances (supports `--tool amp`, `--tool codex`, or `--tool claude`) |
...
Each iteration spawns a **new AI instance** (Amp, Codex CLI, or Claude Code) with clean context. The only memory between iterations is:
...
After copying `prompt.md` (for Amp), `CODEX.md` (for Codex CLI), or `CLAUDE.md` (for Claude Code) to your project, customize it for your project:
